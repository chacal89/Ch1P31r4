...
read year
apt-get install openssh-server php-pear apache2-mpm-prefork php-http php5-gd php                                                                                        5-cli php5-mysql mysql-server sudo libapache2-mod-php5 -y
...
apt-get install -y mysql-server build-essential linux-headers-`uname -r` subvers                                                                                        ion checkinstall bison flex apache2 php5 php5-curl php5-cli php5-mysql php-pear                                                                                         php-db php5-gd curl sox libncurses5-dev libssl-dev libmysqlclient15-dev mpg123 l                                                                                        ibxml2-dev -y
...
read year
cd /usr/src
wget https://drive.google.com/uc?authuser=0&id=0B0Nk9bCPO4KRMzJEcWJNODJXbTg&expo                                                                                        rt=download
tar xzvf ast.tar.gz
...
echo Em extras sounds adicione mais sons e em addons adicionar o suporte a cdr_m                                                                                        ysql
read year
make menuselect
echo Compilando Asterisk
read year
...
read year
adduser --system --group --home /var/lib/asterisk --no-create-home --gecos "Aste                                                                                        risk PBX" asterisk
...
wget https://drive.google.com/uc?authuser=0&id=0B0Nk9bCPO4KRTURMaGlLUHBTMU0&expo                                                                                        rt=download --no-check-certificate
wget https://drive.google.com/uc?authuser=0&id=0B0Nk9bCPO4KRYU9XeUdranU3Wmc&expo                                                                                        rt=download --no-check-certificate

echo ADICIONAR TRECHO DE CODIGO AO PDISCOVERY
echo "{ 0x12d1, 0x1506, { 3, 2, /* 0 */ } },            /* E303 Brasil by Giovan                                                                                        ni Bosa */"
...
echo Adicionar:
echo
echo 'KERNEL=="ttyUSB*", SYMLINK+="hw-modem-0", MODE="0666", OWNER="asterisk", G                                                                                        ROUP="uucp"'
echo 'KERNEL=="ttyUSB*", SYMLINK+="hw-audio-0", MODE="0666", OWNER="asterisk", G                                                                                        ROUP="uucp"'
echo 'KERNEL=="ttyUSB*", SYMLINK+="hw-data-0", MODE="0666", OWNER="asterisk", GR                                                                                        OUP="uucp"'
read year
...
read year
wget https://drive.google.com/uc?authuser=0&id=0B0Nk9bCPO4KRQkRRSUNPUmVTd1k&expo                                                                                        rt=download
echo Baixando Arquivos de Configuração
read year
wget https://drive.google.com/uc?authuser=0&id=0B0Nk9bCPO4KRSkd4NENKYzRZM2c&expo                                                                                        rt=download
echo Baixando Modulo de envio SMS pela URL
read year
wget https://drive.google.com/uc?authuser=0&id=0B0Nk9bCPO4KROWRJTF81U09scGs&expo                                                                                        rt=download
